    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <p className={`text-xs ${m.textSub} font-semibold uppercase tracking-wider flex items-center gap-1`}>
          <Ic n="FlaskConical" size={11} />Checks ({cases.length})
        </p>
        {isFormOpen ? (
          <button onClick={resetCaseForm} className={`text-xs ${m.textSub} hover:text-white transition-colors`}>Cancel</button>
        ) : (
          <button onClick={() => openCaseForm(entry.id)} className="text-xs text-violet-400 hover:text-violet-300 transition-colors">Add Check</button>
        )}
      </div>
      {isFormOpen && (
        <div className={`${m.codeBlock} border ${m.border} rounded-lg p-2.5 flex flex-col gap-3 mb-2`}>
          <div className="flex flex-col gap-1">
            <label className={`text-[10px] font-semibold uppercase tracking-wider ${m.textSub}`}>Check Name</label>
            <input
              className={`${m.input} border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-violet-500 ${m.text}`}
              placeholder="e.g. Happy path — standard user request"
              value={caseTitle}
              onChange={(e) => setCaseTitle(e.target.value)}
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className={`text-[10px] font-semibold uppercase tracking-wider ${m.textSub}`}>Sample Input</label>
            <textarea
              rows={4}
              className={`${m.input} border rounded-lg px-2.5 py-1.5 text-xs resize-none focus:outline-none focus:border-violet-500 ${m.text}`}
              placeholder="The sample request or prompt input to run against this saved prompt"
              value={caseInput}
          ...
          <div className="flex gap-2">
            <div className="flex-1 flex flex-col gap-1">
              <label className={`text-[10px] font-semibold uppercase tracking-wider text-green-400`}>Must Include</label>
              <input
                className={`${m.input} border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-violet-500 ${m.text}`}
                placeholder="keywords, tone, format…"
                value={caseTraits}
                onChange={(e) => setCaseTraits(e.target.value)}
              />
              <span className={`text-[10px] ${m.textMuted}`}>Comma-separated cues the response should include</span>
            </div>
            <div className="flex-1 flex flex-col gap-1">
              <label className={`text-[10px] font-semibold uppercase tracking-wider text-red-400`}>Must Avoid</label>
              <input
                className={`${m.input} border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-violet-500 ${m.text}`}
                placeholder="jargon, filler, hallucination…"
                value={caseExclusions}
                onChange={(e) => setCaseExclusions(e.target.value)}
              />
              <span className={`text-[10px] ${m.textMuted}`}>Comma-separated issues the response should avoid</span>
            </div>
          </div>
          <div className="flex flex-col gap-1">
            <label className={`text-[10px] font-semibold uppercase tracking-wider ${m.textSub}`}>Notes</label>
            <textarea
              rows={2}
              className={`${m.input} border rounded-lg px-2.5 py-1.5 text-xs resize-none focus:outline-none focus:border-violet-500 ${m.text}`}
              placeholder="Why this case matters, edge case context…"
              value={caseNotes}
          ...
          >
            {editingCaseId ? 'Update Check' : 'Save Check'}
          </button>
        </div>
      )}
      {cases.length > 0 ? (
            ...
            <div key={testCase.id} className={`${m.codeBlock} border ${m.border} rounded-lg p-2.5`}>
              <div className="flex items-center justify-between gap-2 mb-1">
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <button onClick={() => loadCaseIntoEditor(testCase)} className={`text-xs ${m.textSub} hover:text-white hover:bg-white/5 rounded-lg px-1.5 py-0.5 transition-colors`}>Load Input</button>
                  <button onClick={() => runSingleCase(testCase, entry.title)} className="text-xs text-blue-400 hover:text-blue-300 hover:bg-white/5 rounded-lg px-1.5 py-0.5 transition-colors">Run Check</button>
              ...
              {(testCase.expectedTraits.length > 0 || testCase.expectedExclusions.length > 0) && (
                <div className={`mt-1 flex flex-wrap gap-2 ${m.textMuted}`}>
                  {testCase.expectedTraits.length > 0 && <span>Must include: {testCase.expectedTraits.join(', ')}</span>}
                  {testCase.expectedExclusions.length > 0 && <span>Must avoid: {testCase.expectedExclusions.join(', ')}</span>}
                </div>
              )}
            </div>
          ))}
        ...
        <p className={`text-xs ${m.textMuted}`}>No saved checks yet.</p>
      )}
    </div>
  );
}
