    ...
    <div className={`fixed inset-0 ${m.modalBg} z-[90] flex items-center justify-center p-3`}>
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="workspace-import-title" aria-describedby="workspace-import-description" tabIndex={-1}
        ...
        {preview.partial && <div role="status" className={`text-sm ${m.textBody}`}>
          ...
          <p>{preview.invalidated
            ? 'This import cannot resume after its destination changed. Closing lets you review another file; previously saved records are not undone.'
            : 'Retry applies the remaining stages. Closing retains this operation in this tab; it does not undo saved records.'}</p>
        </div>}
        <div className="flex flex-wrap gap-2">
          <button type="button" disabled={applying || preview.invalidated || (!preview.partial && !ready)} onClick={preview.partial ? onRetry : onApply}
