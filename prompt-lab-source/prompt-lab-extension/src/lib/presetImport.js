...
function coercePromptToPreset(item, index, options = {}) {
  ...
  return {
    ...
    notes: firstString(item.notes, item.summary),
    resultMeta: isObject(item.resultMeta) ? item.resultMeta : null,
    ...
    completeness: isObject(item.completeness) ? item.completeness : null,
    kind: firstString(item.kind, item.type === 'template' ? 'template' : ''),
    sourceNoteId: firstString(item.sourceNoteId),
    deletedAt: item.deletedAt || null,
    tombstoneVersion: Number.isFinite(item.tombstoneVersion) ? item.tombstoneVersion : 0,
...
export function buildImportEntries(pack, skipIds = new Set(), replaceMap = new Map()) {
  ...
  for (const preset of asArray(pack?.presets)) {
    ...
    const entry = createPromptEntry({
      ...
      notes: ensureString(preset.notes) || ensureString(preset.summary),
      resultMeta: preset.resultMeta || null,
      ...
      completeness: preset.completeness || null,
      kind: ensureString(preset.kind) || 'prompt',
      sourceNoteId: ensureString(preset.sourceNoteId),
      deletedAt: preset.deletedAt || null,
      tombstoneVersion: preset.tombstoneVersion || 0,
      ...
      metadata: {
        ...
        owner: ensureString(preset.metadata?.owner) || ensureString(pack?.title),
        purpose: ensureString(preset.metadata?.purpose) || ensureString(preset.summary),
        status: isDraft ? 'draft' : ensureString(preset.metadata?.status) || ensureString(preset.status) || 'active',
        compatibility: Array.isArray(preset.metadata?.compatibility)
          ? preset.metadata.compatibility
          : Array.isArray(preset.platforms) ? preset.platforms : [],
        riskLevel: ensureString(preset.metadata?.riskLevel),
      },
    });
...
export async function importPresetPack(pack, storageAdapter, { resolutions = {} } = {}) {
  ...
  asArray(normalizedPack.presets).forEach((preset) => {
    ...
    const entry = createPromptEntry({
      ...
      metadata: {
        ...(isObject(preset.metadata) ? preset.metadata : {}),
        owner: ensureString(preset.metadata?.owner) || ensureString(normalizedPack.title),
        purpose: ensureString(preset.metadata?.purpose) || ensureString(preset.summary) || ensureString(normalizedPack.id),
        status: ensureString(preset.metadata?.status) || ensureString(preset.status),
        compatibility: Array.isArray(preset.metadata?.compatibility)
          ? preset.metadata.compatibility
          : Array.isArray(preset.platforms) ? preset.platforms : [],
        riskLevel: ensureString(preset.metadata?.riskLevel),
