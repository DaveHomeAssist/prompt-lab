...
Native acceptance also exercises follow-up generation from a synthetic saved run in the real IndexedDB store. The loopback fixture returns a suggestion derived from that selected output. The test verifies visible generation failure, a rejected Library write, successful retry without another provider call, an unchanged parent, and independent child identity/provenance after native restart and OS uninstall/reinstall. Only upstream synthetic data is seeded; generation and saving use native UI interactions. This does not replace the broader Library import/export matrix or previous-version upgrade/restore checks.
...
WebView2 150 and later intentionally ignore these environment overrides for elevated hosts. `tests/windows-native-standard-user.ps1` removes administrator rights and lowers the installed GUI app to medium integrity. It verifies the actual app token before resuming execution and writes `<phase>-launch-<number>-privileges.json`. The installer, driver, and cleanup coordinator retain the CI runner's normal rights so they can manage their owned processes. This follows Microsoft's [standard-user hosting recommendation](https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/security#recommended-privilege-level-for-webview2-host-applications). It does not modify registry policy, disable OS protections, downgrade WebView2, or add debugging options to the shipped application. Milestone files have `status: running`; only the final phase JSON and successful CI job establish acceptance.

The new restricted token preserves its existing default object access entries and adds the same user's access to its own future kernel objects when missing. The child token is read back before launch. This follows the owned-object access handling in [Chromium's restricted-token implementation](https://chromium.googlesource.com/chromium/src/+/1db4c96ec3d12c0a121325d1b6e63ecd4bbf0ed8/sandbox/win/src/restricted_token.cc); it does not change any existing file, window-station, desktop, or policy ACL.
