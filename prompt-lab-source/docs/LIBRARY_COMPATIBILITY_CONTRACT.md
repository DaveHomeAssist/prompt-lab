...
- If Clear Library or permanent deletion invalidates a partially applied import, remaining stages stop and Retry is disabled. Closing releases the operation so another file can be reviewed. Previously acknowledged stages are not rolled back, and a new import requires fresh confirmation against the current destination.
- Current exports use an object for the pack registry. The import normalizer accepts the empty-array representation emitted by older Prompt Lab schema-2 exports, treating that known absent shape as no registry update so destination packs survive; arbitrary malformed pack registries still fail validation.
